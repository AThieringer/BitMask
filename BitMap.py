class BitMap:
    def __init__(self, length: int, num: int = 0):
        """
        Initialize a BitMap with a specified length and an optional initial integer value.

        :param int length: The maximum number of bits the BitMap can hold.
        :param int num: An integer value to initialize the BitMap. Defaults to 0.
        """
    ...
    def reset_all(self) -> None:
        """Reset all bits in the BitMap to 0."""
        self.value = 0

    def reverse_bits(self) -> None:
        """Reverses the bit order of the BitMap"""
    ...
    def get_count(self) -> int:
        """
        Count the number of set bits (1s) in the BitMap.
    ...
    def set_binary(self, num: str) -> None:
        """
        Set the BitMap value using a binary string.
    ...
    def set_hexadecimal(self, num: str):
        """
        Set the BitMap value using a hexadecimal string.
    ...
    def set_decimal(self, num: int) -> None:
        """
        Set the BitMap value using an integer.
    ...
    def to_binary(self) -> str:
        """Return a binary string representation of the BitMap."""
    ...
    def to_decimal(self) -> int:
        """Return the decimal value of the BitMap."""
        return self.value
    ...
    def __getitem__(self, item):
        if isinstance(item, slice):
            ...
            return BitMap(length, sliced)

        if not isinstance(item, int):
            raise TypeError(f"'BitMap' indices must be integers ot slices, not {type(item)}")
    ...
    def __setitem__(self, key: int, value: int) -> None:
        if not isinstance(key, int):
            if isinstance(key, slice):
                raise TypeError(f"'BitMap' object does not support slicing assignments")
            else:
                raise TypeError(f"'BitMap' indices must be integers, not {type(key)}")
